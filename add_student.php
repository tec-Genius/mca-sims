...
$course_query = mysqli_query($conn,"select * from class where teacher_id='$session_id'") or die(mysqli_error($conn));
$course_row = mysqli_fetch_array($course_query);
$subect_id = $course_row['subject_id'];
?>
<?php
$query_class = mysqli_query($conn,"select * from class where teacher_id='$session_id'") or die(mysqli_error($conn));
                    ...
                    <div class="row-fluid">
                        <div class="span9">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        ...
                                        <select name="student" class="span6" required>
                                            <option></option>
                                            <?php
                                            $query = mysqli_query($conn,"select * from teacher_student where teacher_id = '$session_id'") or die(mysqli_error($conn));
                                            while ($row = mysqli_fetch_array($query)) {
                                                $student_id = $row['student_id'];
                                                $student_query = mysqli_query($conn,"select * from student where student_id='$student_id'") or die(mysqli_error($conn));
                            ...
                            if (isset($_POST['save_subject'])) {
                                ...
                                mysqli_query($conn,"insert into sws (teacher_id,student_id,cys,subject_id,class_id) values('$teacher_id','$student','$cys','$subject','$get_id')") or die(mysqli_error($conn));
                                ?>
