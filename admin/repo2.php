...
$query = mysqli_query($conn,"select distinct stud_year,studsem,student_id,uid,firstname,surname from results where  prog='$prog' and year='$year' and sem='$sem' and stud_year='$cyear'") or die(mysqli_error($conn));
 $total=mysqli_num_rows($query);
		...
		if($sem==1)$TAG= "January-June"; else $TAG="July-December" ;
...
$df->Cell(79,7,'COURSE TITLE',0,0,'L',1);
...
if($cyear==5)
$MYQ = mysqli_query($conn,"select * from results where uid='$uid' and year='$year' and sem='$sem'") or die("here".mysqli_error($conn)); 
              else
$MYQ = mysqli_query($conn,"select * from results where uid='$uid' and year='$year' and sem='$sem'and stud_year='$cyear'") or die("here".mysqli_error($conn));
